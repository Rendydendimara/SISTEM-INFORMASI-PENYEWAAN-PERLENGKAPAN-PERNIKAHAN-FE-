...
import { ApiGetDetailPesanan, ApiUpdateStatusPesanan } from 'src/api/pesanan';
...
const DetailPesananPage = () => {
  ...
  const handleChangeStatus = async (e) => {
    const res = await ApiUpdateStatusPesanan({ status: e.target.value, id: detailPesanan.idPesanan })
    if (res.status === 200) {
      setErrMsg({
        status: 'success',
        msg: `Berhasil ganti status pesanan`,
        isOpen: true
      })
      getPesanan(detailPesanan.idPesanan)
    } else {
      setErrMsg({
        status: 'error',
        msg: `Gagal get data paket. ${res.data.message}`,
                ...
                <CardContent sx={{ pt: 0 }}>
                  <Box>
                    ...
                    <Box display="flex" gap="5px">
                      ...
                      <Link href={`/admin/data-paket/detail?id=${detailPesanan.idPaket}`}>
                        <Typography gutterBottom
                    ...
                    <Box display="flex" alignItems="flex-start" gap="5px">
                      ...
                      <Select
                        ...
                        value={detailPesanan.statusPesanan}
