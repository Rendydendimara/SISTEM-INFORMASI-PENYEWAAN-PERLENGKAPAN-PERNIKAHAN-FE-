...
import { PelangganTable } from 'src/sections/pelanggan/pelanggan-table';
...
const PelangganPage = () => {
  ...
  const getFiltered = () => {
    ...
    if (search) {
      dataReturn = dataReturn.filter((dt) => {
        const haystack = [dt.username.toLowerCase()];
  ...
  const getAllData = async () => {
    ...
    if (res.status === 200) {
      ...
      setErrMsg({
        status: 'error',
        msg: `Gagal get data user. ${res.data.message}`,
        isOpen: true
      })
    }
  }
